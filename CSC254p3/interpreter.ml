...
let primes_prog = "read n cp := 2 while n > 0 found := 0 cf1 := 2 cf1s := cf1 * cf1 while cf1s <= cp cf2 := 2 pr := cf1 * cf2 while pr <= cp if pr == cp found := 1 end cf2 := cf2 + 1 pr := cf1 * cf2 end cf1 := cf1 + 1 cf1s := cf1 * cf1 end if found == 0 write cp n := n - 1 end cp := cp + 1 end";;
...
let rec ast_ize_P (p:parse_tree) : ast_sl =
  ...
  | PT_nt ("P", []) 
        -> []
  | PT_nt ("P", [stmt_list; PT_term "$$"]) 
        -> ast_ize_SL stmt_list
...
and ast_ize_SL (sl:parse_tree) : ast_sl =
  ...
  | PT_nt ("SL", []) 
        -> []
  | PT_nt ("SL",  [stmt; stmt_list])
        -> append [(ast_ize_S stmt)] (ast_ize_SL stmt_list)
...
and ast_ize_S (s:parse_tree) : ast_s =
  ...
  | PT_nt ("S", [PT_term "while"; cond_stmt; stmt_list; PT_term "end"]) 
        -> AST_while ( (ast_ize_C cond_stmt), (ast_ize_SL stmt_list) )
  | PT_nt ("S", [PT_term "if"; cond_stmt; stmt_list; PT_term "end"])
        -> AST_if ( (ast_ize_C cond_stmt), (ast_ize_SL stmt_list))
...
and ast_ize_expr_tail (lhs:ast_e) (tail:parse_tree) :ast_e =
  ...
  | PT_nt ("TT", [])
        -> lhs
  | PT_nt ("TT", [PT_nt ("ao", [PT_term "+"]); term; term_tail]) 
        -> AST_binop ("+", lhs, ast_ize_expr_tail (ast_ize_expr term) term_tail)
  | PT_nt ("TT", [PT_nt ("ao", [PT_term "-"]); term; term_tail]) 
        -> AST_binop ("-", lhs, ast_ize_expr_tail (ast_ize_expr term) term_tail)
  | PT_nt ("FT", []) 
        -> lhs
  | PT_nt ("FT", [PT_nt ("mo", [PT_term "*"]); fact; fact_tail]) 
        ->  AST_binop ("*", lhs, ast_ize_expr_tail (ast_ize_expr fact) fact_tail)
  | PT_nt ("FT", [PT_nt ("mo", [PT_term "/"]); fact; fact_tail]) 
        ->  AST_binop ("/", lhs, ast_ize_expr_tail (ast_ize_expr fact) fact_tail)
...
and ast_ize_C (c:parse_tree) : ast_c =
  match c with
  | PT_nt ("C", [lhs; PT_nt ("rn", [PT_term "<"]); rhs])
...
(* gets integer of a value type *)
let get_val_int (a:value) : int = 
  match a with
  | Value i -> i;;

(* toString for a value type *)
let get_val_string a =
  match a with
  | Value i -> string_of_int i
  | Error i -> i;;

(* gets the value type out of a value * memory tuple *)
let get_val (a, b) = 
  match (a, b) with
  | (a, _) -> a;;

(* input validation *)
let read_val (inp:string) =
  try
    int_of_string inp
  with
  | Failure explanation -> raise (Failure "non-numeric input");;

(* gets the integer value of an id in memory *)
let rec get_val_memory (id:string) (mem:memory) : value =
  match mem with
  | (name, num) :: tail -> if id = name then Value ( num ) else get_val_memory id tail
  | _ -> Error (id ^ ": symbol not found");;

(*print the contents of memory *)
let rec print_memory (mem:memory) =
  match mem with
  | (id, num) :: tail -> print_string (id ^ (string_of_int num)); print_memory tail
  | [] -> print_string "End of Memory";;

let add_vals val1 val2 : value = 
  match (val1, val2) with
  | (Error x, Error y) -> Error ( x ^ " " ^ y )
  | (Error x, Value y) -> Error ( x )
  | (Value x, Error y) -> Error ( y )
  | (Value x, Value y) -> Value ( x + y );;

let sub_vals val1 val2 : value = 
  match (val1, val2) with
  | (Error x, Error y) -> Error ( x ^ " " ^ y )
  | (Error x, Value y) -> Error ( x )
  | (Value x, Error y) -> Error ( y )
...
let div_vals val1 val2 : value = 
  match (val1, val2) with
  | (Error x, Error y) -> Error ( x ^ " " ^ y )
  | (Error x, Value y) -> Error ( x )
  | (Value x, Error y) -> Error ( y )
  | (Value x, Value 0) -> Error ( "divide by zero" )
  | (Value x, Value y) -> Value ( x / y );;

(* translate a bool to an equivalent value type *)
let bool_to_value boolean =
  match boolean with
  | true -> Value 1
  | false -> Value 0;;

(* evaluates the conditional statement *)
let eval_cond op lhs rhs : value =
  match (op, (get_val lhs) , (get_val rhs) ) with
  | (">", Value x, Value y) -> bool_to_value (x > y)
  | ("<", Value x, Value y) -> bool_to_value (x < y)
  | (">=", Value x, Value y) -> bool_to_value (x >= y)
  | ("<=", Value x, Value y) -> bool_to_value (x <= y)
  | ("==", Value x, Value y) -> bool_to_value (x = y)
  | ("!=", Value x, Value y) -> bool_to_value (x != y)
  | (_, Error x, Value y) -> Error (x)
  | (_, Value x, Error y) -> Error (y);;
...
and interpret_sl (sl:ast_sl) (mem:memory)
  ...
  match sl with
  | stmt :: stmt_list 
  -> (start_sl stmt_list (interpret_s stmt mem inp outp))
  | [stmt] 
  -> interpret_s stmt mem inp outp
  | _ -> (true, mem, inp, outp)

(* starts the new statement list using the "mem inp outp" results of the previous interpret_s *)
and start_sl (ast:ast_sl) ((boolean:bool) , (mem:memory) , (inp:string list) , (outp:string list)) 
    : bool * memory * string list * string list =
  match (boolean, mem, inp, outp) with
  | (boolean, mem, inp, outp) -> interpret_sl ast mem inp outp
...
and interpret_assign (lhs:string) (rhs:ast_e) (mem:memory)
                     (inp:string list) (outp:string list)
    : bool * memory * string list * string list =
  (true, (lhs, (get_val_int(get_val(interpret_expr rhs mem))) ) :: mem, inp, outp)

and interpret_read (id:string) (mem:memory)
                   (inp:string list) (outp:string list)
    : bool * memory * string list * string list =
  match inp with
  | [head; tail] -> (true, (id, read_val head) :: mem, [tail], outp)
  | [head] -> (true, (id, read_val head) :: mem, [], outp)
  | _ -> (true, mem, inp, append outp ["unexpected end of input"])

and interpret_write (expr:ast_e) (mem:memory)
                    (inp:string list) (outp:string list)
    : bool * memory * string list * string list =
  (true, mem, inp, append outp [get_val_string (get_val (interpret_expr expr mem))])

and interpret_if (cond:ast_c) (sl:ast_sl) (mem:memory)
                 (inp:string list) (outp:string list)
    : bool * memory * string list * string list =
  match get_val(interpret_cond cond mem) with
...
and interpret_while (cond:ast_c) (sl:ast_sl) (mem:memory)
  ...
  match get_val(interpret_cond cond mem) with
  | Value 0 -> (true, mem, inp, outp)
  | Value 1 -> (start_while cond sl (interpret_sl sl mem inp outp))

(* implements the tail recursion needed for a while loop *)
and start_while (cond:ast_c) (sl:ast_sl) ((boolean:bool), (mem:memory), (inp:string list), (outp:string list))
    : bool * memory * string list * string list =
  match get_val(interpret_cond cond mem) with
  | Value 0 -> (true, mem, inp, outp)
  | Value 1 -> interpret_while cond sl mem inp outp

and interpret_expr (expr:ast_e) (mem:memory) : value * memory =
  match expr with
  | AST_binop ("+", lhs, rhs) -> ( add_vals (get_val (interpret_expr lhs mem)) (get_val (interpret_expr rhs mem)), mem)
  | AST_binop ("-", lhs, rhs) -> ( sub_vals (get_val (interpret_expr lhs mem)) (get_val (interpret_expr rhs mem)), mem)
  | AST_binop ("*", lhs, rhs) -> ( mul_vals (get_val (interpret_expr lhs mem)) (get_val (interpret_expr rhs mem)), mem)
  | AST_binop ("/", lhs, rhs) -> ( div_vals (get_val (interpret_expr lhs mem)) (get_val (interpret_expr rhs mem)), mem)
  | AST_num (num) -> (Value (int_of_string num), mem)
  | AST_id (id) -> ((get_val_memory id mem), mem)

and interpret_cond ((op:string), (lo:ast_e), (ro:ast_e)) (mem:memory)
    : value * memory =
  match op with
  | ">" -> ( (eval_cond ">" (interpret_expr lo mem) (interpret_expr ro mem) ) , mem)
  | "<" -> ( (eval_cond "<" (interpret_expr lo mem) (interpret_expr ro mem) ) , mem)
  | ">=" -> ( (eval_cond ">=" (interpret_expr lo mem) (interpret_expr ro mem) ) , mem)
  | "<=" -> ( (eval_cond "<=" (interpret_expr lo mem) (interpret_expr ro mem) ) , mem)
  | "==" -> ( (eval_cond "==" (interpret_expr lo mem) (interpret_expr ro mem) ) , mem)
