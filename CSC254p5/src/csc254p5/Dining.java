...
class Philosopher extends Thread {
    ...
    private void think() throws ResetException {
        if(Dining.verbose_mode)
        {   
              System.out.println("Philosopher "+ this.index + " thinking");
       }
        ...
        try(FileWriter fw = new FileWriter("myfile.txt", true);
     		...
     		{
     		    out.println("Philosopher "+ this.index + " thinking");
     		} catch (IOException e) {
     		}
    ...
    //To solve the deadlock problem, I implemented a method to seperate the philosophers with odd index and even index 
    //with odd index they can only access left fork first and
    // the philosopher with even index can only access right fork first
    //I use synchronized keyword to prevent concurrent issues
    private void hunger() throws ResetException {
        ...
        if(Dining.verbose_mode)
        {   
              System.out.println("Philosopher "+ this.index + " waiting");
       }
        try(FileWriter fw = new FileWriter("myfile.txt", true);
     		    BufferedWriter bw = new BufferedWriter(fw);
     		    PrintWriter out = new PrintWriter(bw))
     		{
     		    out.println("Philosopher "+ this.index + " waiting");
     		} catch (IOException e) {
     		}
        if(this.index%2==1){
            synchronized(left_fork)
            {
                if(!left_fork.usestate)
                {
                    left_fork.acquire(x, y);
                    left_fork.user=this.index;
                    yield();
                    synchronized(right_fork)
                    {
                        if(right_fork.usestate)
                        {
                        	left_fork.release();
                        }
                        else
    ...
    private void eat() throws ResetException {
        ...
        if(left_fork.usestate = true && 
        	right_fork.usestate == true && 
        	left_fork.user == this.index && 
        	right_fork.user == this.index)
        {
             if(Dining.verbose_mode)
             {   
                   System.out.println("Philosopher "+ this.index + " eating");
            }
             ...
             try(FileWriter fw = new FileWriter("myfile.txt", true);
         		    ...
         		    PrintWriter out = new PrintWriter(bw))
         		{
         		    out.println("Philosopher "+ this.index + " eating");
         		} catch (IOException e) {
         		}
        color = EAT_COLOR;
        t.repaint();
